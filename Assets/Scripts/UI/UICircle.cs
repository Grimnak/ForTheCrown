...
public class UICircle : RawImage
{
    ...
    [SerializeField]
    [Range(4, 360)]
    int _segments = 4;  //edges, >= 25 -> circle
    ...
    public void texSelector(int idx)
    {
        if (idx >= texPool.Length)
        {
            Debug.Log("This unit information is not in the global script for look up");
            return;
        }
        this.texture = texPool[idx];
    }
}
}